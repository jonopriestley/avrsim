class Token {
    constructor(tok_type, value = null, location = null) {
        this.type = tok_type;
        this.value = value;
...
class Argument {
    ...
    isLegalArgument(tok, line_num) {
        // CHECK IF IT'S A LEGAL ARGUMENT
        const legal_token_types = this.getTokenType(); // the legal token types for this argument

        if (legal_token_types.includes(tok.getType())) return;

        const reg_str = (tok.getType() === 'REG') ? 'R' : '' ;
        this.newError(`Illegal token '${reg_str}${tok.getValue()}' on line ${line_num}.`);
        
    }

    isWithinLegalBounds(tok, line_num) {
        // CHECK ITS LEGAL TOKEN TYPE AND WITHIN THE LEGAL BOUNDS
        if (!this.hasValueRange()) return true;

        const val = (tok.getType() === 'WORDPLUSQ') ? parseInt(tok.getValue().substring(2)) : tok.getValue(); // get rid of the Z+ or Y+ part if needed
        
        if (tok.getType() === 'REF' || (this.getMinVal() <= val && val <= this.getMaxVal())) return true;
            
        const reg_str = (tok.getType() === 'REG') ? 'R' : '' ;
        this.newError(`Illegal argument '${reg_str}${tok.getValue()}' on line ${line_num}.`);
    }

    isInLegalList(tok, line_num) {
        // CHECK ITS LEGAL TOKEN TYPE AND IN THE LEGAL LIST
        const val_not_in_opts = this.hasOptionsList() && !this.getOptionsList().includes(tok.getValue());
        const has_wrong_val = this.hasExactValue() && (tok.getValue() !== this.getExactValue());
        const has_illegal_val = has_wrong_val || (val_not_in_opts && !this.hasValueRange());

        if (!has_illegal_val) return;
    ...
    isLegalToken(tok, line_num) {
        ...
        this.isLegalArgument(tok, line_num);
        this.isWithinLegalBounds(tok, line_num);
        this.isInLegalList(tok, line_num); 
    }
...
class Instruction {
    constructor(tokens) {
        ...
        this.all_opcodes = {
            ...
            'SBIC': ['A', 'b', '10011001AAAAAbbb'],
            'SBIS': ['A', 'b', '10011011AAAAAbbb'],
            'SBIW': null,
    ...
    makeOpcode() {
        ...
        // More tricky opcodes
        if (opcode_requirements === null) return this.makeComplexOpcode(inst);
        
        // Simple opcodes
        return this.makeDefinedOpcode(opcode_requirements);

        
    }

    makeDefinedOpcode(opcode_requirements) {
    ...
    makeComplexOpcode(inst) {
        ...
        switch (inst) {
            ...
            case 'LD':
                this.makeDataTransferOpcode(0, 0);
            case 'LDD':
                this.makeDataTransferOpcode(0, 1);
            case 'LPM':
                if (this.getArgs().length === 0) return '1001010111001000';
                w = this.args[1].getValue();
                d = this.binLenDigits(this.args[0].getValue(), 5);
                if      (w === 'Z')  return `1001000${d}0100`;
                else if (w === 'Z+') return `1001000${d}0101`;
            case 'LSL':
                d = this.binLenDigits(this.args[0].getValue(), 5);
            ...
            case 'ST':
                this.makeDataTransferOpcode(1, 0);
            case 'STD':
                this.makeDataTransferOpcode(1, 1);
            case 'TST':
                d = this.binLenDigits(this.args[0].getValue(), 5);
    ...
    makeDataTransferOpcode(ld_st, has_displacement) {
        const d = this.binLenDigits(this.args[ld_st].getValue(), 5);
        const w = this.args[1 - ld_st].getValue();

        if (has_displacement) {
            const q = this.binLenDigits(parseInt(this.args[1 - ld_st].getValue().slice(2)), 6);
            const yz = (w === 'Y') ? '1' : '0';
            return `10${q.slice(0, 1)}0${q.slice(1, 3)}${ld_st}${d}${yz}${q.slice(3)}`;
        }

        const x_yz = (['Y', 'Z'].includes(w)) ? '0' : '1';                              // Y and Z have a 0 there, the rest have a 1
        const word = (w.includes('X')) ? '11' : (w.includes('Y')) ? '10' : '00';        // X = 11, Y = 10, Z = 00
        const plus_minus = (w.includes('+')) ? '01' : (w.includes('-')) ? '10' : '00';  // + = 01, - = 10, nothing = 00

        return `100${x_yz}00${ld_st}${d}${word}${plus_minus}`;
    }
    ...
    toString(base) {
        ...
        if (argsLen === 0) return inst;
        
        let arg1 = this.args[0].getValue();

        if (this.operands.getInstructionArg(inst, 0).getTokenType().includes('REG')) arg1 = `R${arg1}`;
        else if (this.args[0].getType() === 'INT')                                   arg1 = arg1.toString(base);

        if (argsLen === 1) return `${inst} ${arg1}`;

        let arg2 = this.args[1].getValue();

        if (this.operands.getOperands(inst)[1].getTokenType().includes('REG')) arg2 = `R${arg2}`;
        else if (this.args[1].getType() === 'INT')                             arg2 = arg2.toString(base);
    ...
    getArgValue(arg_num) {
        return this.line.getArgs()[arg_num].getValue();
    }
...
class Lexer {
    ...
    getPatterns() {
        return [
            ...
            [/^-{0,1}\$[\dA-Fa-f]+/, 'INTHEX1'],    // numbers
            [/^-{0,1}0[xX][\dA-Fa-f]+/, 'INTHEX2'], // numbers
            [/^-{0,1}0[oO][0-7]+/, 'INTOCT'],       // numbers
            [/^-{0,1}0[bB][01]+/, 'INTBIN'],        // numbers
            [/^-{0,1}\d+/, 'INT'],                  // numbers
            [/^[a-zA-Z]{2,6}/, 'REF'],         // instructions → CAN TURN REFS USED IN AN INSTRUCTION INTO INST TYPE
            [/^\".*?\"/, 'STR'],                // string
            [/^\'.*?\'/, 'STR'],                // string
            [/^“.*?”/, 'STR'],                  // string
            [/^‘.*?’/, 'STR'],                  // string
    ...
    tokenise() {
        ...
        for (let line_number = 0; line_number < this.getCodeArray().length; line_number++) {
            this.getLineManager().setLineNumber(line_number);
            this.tokeniseLine();
            this.fixBadTokens();
    ...
    tokeniseLine() {
        ...
        const line = this.getCodeArray()[this.getLineManager().getLineNumber()] + ' ';
        this.getLineManager().clearLine();
        
        // Iterate over the input code, finding matches for each token type
        let start_pos = 0;   // location in the line
        while (start_pos < line.length) {
            start_pos += this.getMatch(line, start_pos);
        }
    }

    getMatch(line, start_pos) {
        ...
        for (let i = 0; i < this.patterns.length; i++) {
            ...
            this.createNewToken(tag, match, start_pos);
            break;
        }

        if (match) return match[0].length;
        
        this.newError(`Invalid syntax on line ${this.getLineManager().getLineNumber() + 1} starting at position ${position}.`);
    }

    createNewToken(tag, match, start_pos) {
        if (!tag) return;

        const token = new Token(tag, match[0], `${this.getLineManager().getLineNumber() + 1}:${start_pos + 1}`);
        this.getLineManager().pushToLine(token);
    }
    ...
    fixBadTokens() {
        ...
        let i = 0;
        while (i < this.getLineManager().getLineLength()) {
            this.getTokenManager().setToken(i);

            this.makeInstructionTokens(i);
            this.makeRegTokens();
            this.trimLabels();
            this.capitaliseDirectives();
            this.tokeniseInt();

            i = this.combineRefs(i);
            i += 1;
        }
    }
    ...
    tokeniseInt() {
        ...
        if (!this.getTokenManager().getType().includes('INT')) return;

        const base = this.getIntBase();
        const slice_loc = (this.getTokenManager().getType() === 'INT') ? 0 : (this.getTokenManager().getType() === 'INTHEX1') ? 1 : 2;
        const is_neg = (this.getTokenManager().getValue().includes('-'));
        
        // Remove negative and store as multiplier for down the bottom of the function
        if (is_neg) this.getTokenManager().setValue(this.getTokenManager().getValue().slice(1));
        
        const neg = is_neg ? -1 : 1;
        const int_value = parseInt(this.getTokenManager().getValue().slice(slice_loc), base);

        this.getTokenManager().setType('INT');
        this.getTokenManager().setValue(neg * int_value);
    }
    ...
    trimLabels() {
        if (this.getTokenManager().getType() !== 'LABEL') return;
        
        const label_end = this.getTokenManager().getValue().length - 1;
        const new_value = this.getTokenManager().getValue().slice(0, label_end).trim(); // remove the colon at the end (and any whitespace)
        this.getTokenManager().setValue(new_value); 
    }

    capitaliseDirectives() {
        ...
        this.getTokenManager().setValue(directive);
    }

    makeInstructionTokens(i) {
        if (this.getTokenManager().getType() !== 'REF') return;
        
        const insts = new InstructionSet();
        const upper_string = this.getTokenManager().getValue().toUpperCase();
        const is_inst = insts.inst_set.includes(upper_string);
        if (!is_inst) return;

        const is_after_label = (i === 1 && this.getLineManager().getLine(0).getType() === 'LABEL');
        if (i !== 0 && !is_after_label) return;
        
        this.getTokenManager().setType('INST');
        this.getTokenManager().setValue(upper_string);
    }

    makeRegTokens() {
        if (this.getTokenManager().getType() !== 'REG') return;
        
        this.getTokenManager().setValue(parseInt(this.getTokenManager().getValue().substring(1)));   // turn REG:Rn into REG:n
        if (this.getTokenManager().getValue() <= 31) return;

        this.newError(`Illegal register \'R${this.getTokenManager().getValue()}\' on line ${this.getTokenManager().getToken().getLine()}.`);
    }
    ...
    combineRefs(i) {
        ...
        this.getLineManager().getLine(i - 1).setValue(combined_value); // put them together
        this.getLineManager().getLine().splice(i, 1); // remove the current one
        return i - 1;   // counter the later shift forward
    }

    getTokenManager() {
        return this.tok_manager;
    }
    ...
    getLineManager() {
        return this.getTokenManager().getLineManager();
    }
    
    toString() {
        let s = '';
        let line;
        for (let l = 0; l < this.getLineManager().getNumLines(); l++) {
            line = this.getLineManager().getAllLines(l);
            line = Array.from({ length: line.length }, (_, tok) => line[tok].toString());
            s += line.join('\n') + '\n';
        }
        return s;
    }
...
class Parser {
    ...
    newData(tok_manager, txt) {
        
        this.tok_manager = tok_manager;

        this.defs = new ProgramDefinitions(); 
        this.break_point = new BreakPoint();
        ...
        for (let i = this.getPMEM().getLength(); i <= this.getPMEM().getFlashEnd(); i++) {
            this.getPMEM().push(new Instruction([new Token('INST', 'NOP')]));
        }
    ...
    replaceDirectiveRefs() {
        if (['.BYTE','.WORD', '.SPACE'].includes(this.line_directive)) this.replaceRefs(this.getLineManager().getLine());
        else if (['.SET','.EQU'].includes(this.line_directive)) this.replaceRefs(this.getLineManager().getLine().slice(2));        
        else return;
    ...
    setRefToInt(current_tok) {
        ...
        if (this.getProgDefs().getLabels()[current_tok.getValue()] !== undefined) current_tok.setValue(this.getProgDefs().getLabels()[current_tok.getValue()]);
        else if (this.getProgDefs().getEqus()[current_tok.getValue()] !== undefined) current_tok.setValue(this.getProgDefs().getEqus()[current_tok.getValue()]);
        else this.newError(`Invalid reference \'${current_tok.getValue()}\' on line ${this.lineInFile()}.`);

        current_tok.setType('INT');
    }

    evaluateCurrentLine() {
        let line = this.getLineManager().getLine();
        line = this.evaluator.evaluateNewLine(line);
        this.getLineManager().setLine(line);
    }
    ...
    checkFirstLine() {
        const first_line = this.getLineManager().getAllLines(0); 
    ...
    checkLastLine() {
        ...
        const last_line_loc = this.getLineManager().getNumLines() - 1;
        const last_line = this.getLineManager().getAllLines(last_line_loc);
    ...
    findTextSectionStart() {
        for (this.getLineManager().setLineNumber(0); this.getLineManager().getLineNumber() < this.getLineManager().getNumLines(); this.getLineManager().offsetLineNumber(1)) {
            this.getLineManager().setLine();

            // If you find a .SECTION directive check it
            if (this.getLineManager().getLine(0).getValue() !== '.SECTION') continue;
            if (this.getLineManager().getLine(0).getType() !== 'DIR') continue;
            
            const bad_length = this.getLineManager().getLineLength() !== 2;

            const no_2nd_directive  = bad_length || this.getLineManager().getLine(1).getType() !== 'DIR';

            const bad_2nd_directive = no_2nd_directive || !['.DATA', '.TEXT'].includes(this.getLineManager().getLine(1).getValue());

            if (bad_2nd_directive) this.newError(`Invalid '.section' directive on line ${this.lineInFile()}.`);

            // If you find the text section then stop looking
            if (this.getLineManager().getLine(1).getValue() === '.TEXT') {
                this.getLineManager().setTextSecStart(this.getLineManager().getLineNumber());
                break;
            }

        }

        if (this.getLineManager().getTextSecStart() !== null) return;
        
        this.newError("File must contain a '.section .text'");  // If there's no text section
    }
    ...
    validateInstructions() {
        for (this.getLineManager().setLineNumber(0); this.getLineManager().getLineNumber() < this.getLineManager().getNumLines(); this.getLineManager().offsetLineNumber(1)) {
            ...
            for (this.getTokenManager().setTokNum(0); this.getTokenManager().getTokNum() < this.getLineManager().getLineLength(); this.getTokenManager().offsetTokNum(1)) {
                this.getTokenManager().setToken();

                // Check INST tokens are real instructions
                if (this.getTokenManager().getType() !== 'INST') continue;
                if (this.inst_set.isInstruction(this.getTokenManager().getValue())) continue;
                
                this.newError(`Invalid instruction \'${this.getTokenManager().getValue()}\' on line ${this.lineInFile()}.`);
    ...
    defineDefs() {
        ...
        for (let [key, value] of Object.entries(existing_defs)) {
            this.getProgDefs().newDef(key, value, 0);
        }
    }
    ...
    defineEqus() {
        ...
        for (let [key, value] of Object.entries(existing_equs)) {
            this.getProgDefs().newEqu(key, value, 0);
        }
    }

    checkGlobalAtStart() {
        this.getLineManager().setLine(null, this.getLineManager().getTextSecStart() + 1);

        const bad_line_length = this.getLineManager().getLineLength()       !== 2;
        const bad_line_val    = this.getLineManager().getLine(0).getValue() !== '.GLOBAL';

        if (bad_line_length || bad_line_val) this.newError(`Must begin text section with a valid .global directive: line ${this.lineInFile()}.`);

        this.global_funct_names = [this.getLineManager().getLine(1).getValue()];        // the name of the global function for later
    }

    createPMEM() {
        const start_line = this.getLineManager().getTextSecStart() + 2;
        ...
        while (this.getLineManager().getLineNumber() < (num_lines - 1)) {
            ...
            if (this.getLineManager().getLine(0).getType() !== 'INST') {
                this.newError(`Illegal token \'${this.getLineManager().getLine(0).getValue()}\' on line ${this.lineInFile()}.`);
            }

            // Set the break point to be the line equal about to be put in
            this.getBreakPoint().defineBreakPoint( this.getPMEM().getLength(), this.lineInFile() );
    ...
    removeLabels(mem_type) {
        const loc = (mem_type === 'pmem') ? this.getPMEM().getLength() : this.getDMEM().getLength();
        
        // Check for labels and remove them
        this.getTokenManager().setTokNum(0);
        while (this.getTokenManager().getTokNum() < this.getLineManager().getLineLength()) {
    ...
    defineGlobalFunctions() {
        ...
        if (this.getLineManager().getLine(0).getType() !== 'DIR' || this.getLineManager().getLine(0).getValue() !== '.GLOBAL') {
            return false;
        }

        if (this.getPMEM().getLength() > 0) {
            this.newError(`Illegal .global directive on line ${this.lineInFile()}.`);
        }

        if (this.getLineManager().getLineLength() !== 2) {
            this.newError(`Incorrect number of arguments for .global directive on line ${this.lineInFile()}.`);
        }

        if (this.getLineManager().getLine(1).getType() !== 'REF') {
            this.newError(`Illegal token type ${this.getLineManager().getLine(1).getType()} for the argument ${this.getLineManager().getLine(1).getValue()} on line ${this.lineInFile()}.`);
        }
    ...
    checkInstructionSyntax() {
        ...
        let current_line;
        for (this.getLineManager().setLineNumber(0); this.getLineManager().getLineNumber() < this.getPMEM().getLength(); this.getLineManager().offsetLineNumber(1)) {
            
            current_line = this.getPMEM().getValue(this.getLineManager().getLineNumber());
            if (current_line === null) continue;

            this.getLineManager().setLine(current_line);   // the line up to
    ...
    checkInstructionExists(inst) {
        ...
        this.newError(`Illegal instruction \'${inst}\' on line ${this.lineInFile()}.`);
    }
    ...
    checkDataSection() {
        ...
        if (this.getDMEM().getLength() <= this.getDMEM().getRAMEnd() + 1) return;
    ...
    executeDirectives() {
        while (this.getTokenManager().getTokNum() < this.getLineManager().getLineLength()) {
            ...
            this.executeByteDirective();
            this.executeAsciiDirectives();  // String, Ascii, Asciz directives
            this.executeSpaceDirective();
            this.executeSetDirectives();    // Set, Equ directives
            this.checkDirectiveCommas();

            this.getTokenManager().offsetTokNum(1);
        }
    }
    ...
    executeByteDirective() {
        ...
        if (this.getTokenManager().getType() !== 'INT') {
            const start = this.getTokenManager().getToken().getStart();
    ...
    executeAsciiDirectives() {
        // String, Ascii, Asciz directives
        const parity_of_current_tok = this.getTokenManager().getTokNum() & 1;
        const is_ascii = ['.STRING', '.ASCII', '.ASCIZ'].includes(this.line_directive);
        
        if (parity_of_current_tok !== 1 || !is_ascii) return;

        let string_text = this.getTokenManager().getValue();
        
        if (this.getTokenManager().getType() !== 'STR') {
            this.newError(`Bad token \'${string_text}\' on line ${this.lineInFile()}.`);
        }

        string_text = string_text.slice(1, (string_text.length - 1));   // remove quotation marks from either side

        let char_ascii_value, char;
            ...
            if (!escape) {
                ...
                if (char === '\\') {
                    ...
                    if (i + 1 === string_text.length) {
                        this.newError(`Invalid escape character \'${char}\' on line ${this.lineInFile()}.`);
                    }
                    continue;
                }
            }
    ...
    executeSpaceDirective() {
        ...
        if (parity_of_current_tok !== 1 || this.line_directive !== '.SPACE') {
            return;
        }

        const tok_val = this.getTokenManager().getValue();
        
        if (this.getTokenManager().getType() !== 'INT') { // expecting integer
            this.newError(`Bad token \'${tok_val}\' on line ${this.lineInFile()}.`);
        }
        
        const tok_num          = this.getTokenManager().getTokNum();
        const is_last_tok      = this.getLineManager().getLineLength() === (tok_num + 1);
        const is_num_spaces    = this.getLineManager().getLine(tok_num - 1).getType() === 'DIR';
        
        // Assume only 1 arg given (number of spaces)
        let   space_value      = 0;
        let   number_of_spaces = tok_val;

        // Check it's the final token. If it's the second argument given it must be the last
        if (!is_last_tok) {
            if (is_num_spaces) return;   // Only execute once the whole line has been read
            this.newError(`Too many arguments given for .string on line ${this.lineInFile()}.`);
        }

        // If it's the value of the spaces
        if (!is_num_spaces) {
            space_value = tok_val;
            number_of_spaces = this.getLineManager().getLine(tok_num - 2).getValue();
        }
    ...
    executeSetDirectives() {
        const parity_of_current_tok = this.getTokenManager().getTokNum() & 1;

        if (!['.EQU','.SET'].includes(this.line_directive)) return;

        // Check the number of arguments
        if ((this.getLineManager().getLineLength() - this.getTokenManager().getTokNum()) !== 3) { // if there's too many arguments
            this.newError(`Wrong number of arguments given for .equ on line ${this.lineInFile()}.`);
        }

        // if it's the 3rd last argument (expecting REF)
        if (this.getTokenManager().getType() !== 'REF') {
            this.newError(`Bad argument \'${this.getTokenManager().getValue()}\' on line ${this.lineInFile()}.`);
        }
        ...
        if (this.getTokenManager().getType() !== 'COMMA') {
            this.newError(`Bad argument \'${this.getTokenManager().getValue()}\' on line ${this.lineInFile()}.`);
        }
        ...
        this.asciiCharacterLiteral();

        if (this.getTokenManager().getType() !== 'INT') {
            this.newError(`Bad argument \'${this.getTokenManager().getValue()}\' on line ${this.lineInFile()}.`);
        }

        const equ_word = this.getLineManager().getLine(this.getTokenManager().getTokNum() - 2).getValue();  // get the equ name for the labels list
        const tok_val = this.getTokenManager().getValue();
        
        this.getProgDefs().newEqu(equ_word, tok_val, this.lineInFile());
    }

    asciiCharacterLiteral() {
        // Support ASCII character literals
        if (this.getTogetTokenManager().getTypekType() !== 'STR') return;

        const val = this.getTokenManager().getValue();

        if (val.length !== 3 || val[0] !== val[2]) return;

        const str_matcher = /^[\"\'“‘].*?[\"\'”’]/;
        const match = str_matcher.exec(val);

        if (!match) this.newError(`Bad argument \'${this.getTokenManager().getValue()}\' on line ${this.lineInFile()}.`);;

        const charCode = val.charCodeAt(1);
        this.getTokenManager().setType('INT');
        this.getTokenManager().setValue(charCode); 
    }

    checkDirectiveCommas() {
        // Should be comma if there are even number of tokens left. Raise error.
        const parity_of_current_tok = this.getTokenManager().getTokNum() & 1;

        const start = this.getTokenManager().getToken().getStart();

        if (parity_of_current_tok === 0 && this.getTokenManager().getType() !== 'COMMA') {
            this.newError(`Missing comma on line ${this.lineInFile()} at position ${start}.`);
        }

        else if ((this.getTokenManager().getTokNum() + 1 === this.getLineManager().getLineLength()) && this.getTokenManager().getType() === 'COMMA') {
            this.newError(`Illegal comma on line ${this.lineInFile()}  at position ${start}.`);
        }
    }
    ...
    evaluateLineExpressions() {
        ...
        let isRef, isHiLo, tok_type;

        for (this.getTokenManager().setTokNum(0); this.getTokenManager().getTokNum() < this.getLineManager().getLineLength(); this.getTokenManager().offsetTokNum(1)) {
            this.getTokenManager().setToken();  // set current token

            tok_type = this.getTokenManager().getType();
            isRef = tok_type === 'REF';
            isHiLo = ['HI8', 'LO8'].includes(tok_type);
            
            if (isHiLo) this.evaluateHiLo();    // Replace hi8/lo8 with its value
            else if (isRef) this.evaluateRef(); // Replace REF with its value
        }
    ...
    evaluateHiLo() {
        const fnct = this.getTokenManager().getType(); // store the function name while you're pointed at it so you can use it later

        // Check there's enough room
        if (this.getLineManager().getLineLength() <= this.getTokenManager().getTokNum() + 3) {
            this.newError(`Bad syntax on line ${this.lineInFile()}. Not enough tokens given for a valid ${this.getTokenManager().getType()} function.`); 
        }

        this.getTokenManager().offsetTokNum(1);

        // Check the token we expect to be the left bracket
        if (this.getLineManager().getLine(this.getTokenManager().getTokNum()).getType() !== 'LPAR') {
            this.newError(`Illegal ${this.getTokenManager().getValue()} left bracket on line ${this.lineInFile()}.`);
        }

        this.getTokenManager().offsetTokNum(1);

        const expr_start = this.getTokenManager().getTokNum();

        this.checkBracketsClose();

        this.replaceRefs(this.getLineManager().getLine().slice(expr_start, this.getTokenManager().getTokNum() - 1));
        
        this.evaluateCurrentLine();

        this.getTokenManager().setTokNum(expr_start - 1);
        const val = this.getLineManager().getLine(this.getTokenManager().getTokNum()).getValue();

        if (val > 0x7fffffff || val < -0x80000000) this.newError(`Error: bignum invalid on line ${this.lineInFile()}. Cannot have a number outside of [-2^31, 2^31 - 1].`);

        // Convert the value to the hi8/lo8 value
        let int_value = (fnct === 'HI8') ? this.hi8(val) : this.lo8(val);

        this.getLineManager().getLine().splice(expr_start - 2, 2, new Token('INT', int_value, this.getTokenManager().getToken().getLocation()));
        this.getTokenManager().setTokNum(expr_start - 2);
    }

    evaluateRef() {
        let tok_type = 'INT';
        let tok_val = this.getTokenManager().getValue();

        const first_tok_val = this.getLineManager().getLine(0).getValue();  // first token in the line
        ...
        if (this.inst_set.isNonrelativeControlFlowInstruction(first_tok_val) && !this.functions.isFunction(tok_val)) {
            tok_val = this.getProgDefs().getLabels()[tok_val];      // Get k for label
        }

        // If it's a relative control flow instruction
        else if (this.inst_set.isRelativeControlFlowInstruction(first_tok_val)) {
            tok_val = this.getProgDefs().getLabels()[tok_val] - 1 - this.getLineManager().getLineNumber();
        }

        // If it's in data labels
        else if (this.getProgDefs().getLabels()[tok_val] !== undefined) {
            tok_val = this.getProgDefs().getLabels()[tok_val];
        }

        // If it's a REG definition
        else if (this.getProgDefs().getDefs()[tok_val] !== undefined) {
            tok_type = 'REG';
            tok_val = this.getProgDefs().getDefs()[tok_val];
        }

        // If it's a REG definition
        else if (this.getProgDefs().getEqus()[tok_val] !== undefined) {
            tok_val = this.getProgDefs().getEqus()[tok_val];
        }
        ...
        // If it's none of these and not a function, raise an error
            ...
            this.newError(`Bad reference ${tok_val} on line ${this.lineInFile()}.`);
        }

        this.getTokenManager().setValue(tok_val);
        this.getTokenManager().setType(tok_type);
    }
    ...
    checkBracketsClose() {
        ...
        while (this.getTokenManager().getTokNum() < this.getLineManager().getLineLength()) {
            ...
            bracket_level += (this.getTokenManager().getType() === 'LPAR') ? 1 : (this.getTokenManager().getType() === 'RPAR') ? -1 : 0;
            this.getTokenManager().offsetTokNum(1);
        }

        if (bracket_level !== 0) this.newError(`Missing ')' on line ${this.lineInFile()}.`);
    }
    ...
    lineInFile() {
        return this.getLineManager().getLineInFile();
    }
...
class Interpreter {
    ...
    emptyData() {
        this.pmem = new PMEM();
        this.dmem = new DMEM();
        this.prog_data = new ProgramData();
        this.operands = new Operands();
    ...
    newData(pmem, dmem, break_point) {
        ...
        const pch = new Register('PCH', this.getDMEM().getValue(0x5c));
        this.getDMEM().setValue(0x5b, 0); // PC was stored in these two previously to easily transfer the information
        this.getDMEM().setValue(0x5c, 0);
        this.pc = new PC(pcl, pch);

        // SETTING SREG = RAMEND
        this.getDMEM().setSP(this.getDMEM().getRAMEnd()); 
    }
    ...
    step() {
        ...
        switch (inst) {
            ...
            case 'ADD':
                ...
                break;
            case 'ADIW':
                this.immediateWithWord();
                break;
            case 'AND':
                this.bitwiseOperation('AND');
                break;
            case 'ANDI':
                this.bitwiseOperation('AND');
                break;
            case 'ASR':
                ...
                break;
            case 'BCLR':
                s = this.getArgValue(0);
                this.getDMEM().updateSREGBit(0, s);   // Clear bit s
                break;
            case 'BLD':
                Rd = this.getArgValue(0);
                b = this.getArgValue(1);
                T_bit = this.getDMEM().getSREGBit(6);
                Result = T_bit ? Rd | ( 1 << b ) : Rd & ( 0xff - ( 1 << b ) );
                this.writeResultToRd(Result);   // Rd(b) <-- T 
                break;
            case 'BRBC':
                s = this.getArgValue(0);
                this.branch(s, 0, 1);
            ...
            case 'BSET':
                s = this.getArgValue(0);
                this.getDMEM().updateSREGBit(1, s);   // Set bit s
                break;
            case 'BST':
                Rd = this.getArgValue(0);
                b = this.getArgValue(1);
                T_bit = this.getBit(Rd, b);
                this.getDMEM().updateSREGBit(T_bit, 6);
                break;
            case 'CALL':
                const type_arg0 = this.line.getArgs()[0].getType();
                if (type_arg0 === 'INT') this.call();
                if (this.getArgValue(0) === 'printf') this.printf();    // OTHERWISE IF IT'S PRINTF
                break;
            case 'CBI':
                this.bitInIO(0, 0);
                break;
            case 'CBR':
                this.bitwiseOperation('CBR');
                break;
            case 'CLC':
                this.getDMEM().updateSREGBit(0, 0);
                break;
            case 'CLH':
                this.getDMEM().updateSREGBit(0, 5);
                break;
            case 'CLI':
                this.getDMEM().updateSREGBit(0, 7);
                break;
            case 'CLN':
                this.getDMEM().updateSREGBit(0, 2);
                break;
            case 'CLR':
                this.writeResultToRd(0);  // Rd <-- 0
                this.getDMEM().updateSREG(null, null, null, 0, 0, 0, 1, null);
                break;
            case 'CLS':
                this.getDMEM().updateSREGBit(0, 4);
                break;
            case 'CLT':
                this.getDMEM().updateSREGBit(0, 6);
                break;
            case 'CLV':
                this.getDMEM().updateSREGBit(0, 3);
                break;
            case 'CLZ':
                this.getDMEM().updateSREGBit(0, 1);
                break;
            case 'COM':
                this.bitwiseOperation('COM');
                break;
            case 'CP':
                this.sub(0, 1);
                break;
            ...
            case 'CPSE':
                ...
                this.getProgramData().offsetBranchesSeen(1);
                if (Rd !== Rr) break;
                this.getProgramData().offsetBranchesTaken(1);   // branches taken += 1
                this.skip();
                break;
            case 'DEC':
                this.incDec(1);
            ...
            case 'ICALL':
                ...
                break;
            case 'IJMP':
                this.getPC().setValue(this.getDMEM().getWord(30) - 1);
                this.getProgramData().offsetCycles(1);
                break;
            case 'IN':
                A_val = this.getArgValue(1);
                Result = this.getDMEM().getValue(A_val + 0x20);
                this.writeResultToRd(Result);   // Rd <-- I/O(A)
                break;
            case 'INC':
                this.incDec();
                break;
            case 'JMP':
                this.getPC().setValue(this.getArgValue(0) - 1);
                this.getProgramData().offsetCycles(2);
                break;
            case 'LD':
                this.loadStore(0);
                break;
            case 'LDD':
                this.loadStoreDisplacement(0);
                break;
            case 'LDI':
                K_val = this.getArgValue(1);
                this.writeResultToRd(K_val);  // Rd <-- K
                break;
            case 'LDS':
                this.loadStoreDataSpace(0);
                break;
            case 'LPM':
                k = this.getDMEM().getWord(30);
                Rd = (this.line.getArgs().length === 0) ? 0 : this.getArgValue(0);      // set Rd as either 0 or whatever it's specified as
                if (this.line.getArgs().length !== 0 && this.getArgValue(1).includes('+')) this.getDMEM().offsetWord(30, 1);

                const inst_location = (k >> 1) & 0xffff;
                const inst_opcode = this.getPMEM().getValue(inst_location).getOpcode();
                const start = 8 * !(k & 1); // slicing at 0 gives the most significant 8 bits, so k = 1 returns 0
                const opcode_slice = inst_opcode.slice(start, start + 8);

                this.getDMEM().setValue(Rd, parseInt(opcode_slice, 2));
                this.getProgramData().offsetCycles(2);
                break;
            case 'LSL':
                this.add(0, 1);
            ...
            case 'NEG':
                ...
                this.writeResultToRd(Result);   // Rd <-- 0 - Rd

                H_bit = this.getBit(Result, 3) | this.getNotBit(Rd, 3);
                V_bit = (Result === 128);
                N_bit = this.getBit(Result, 7);
                this.getDMEM().updateSREG(null, null, H_bit, N_bit ^ V_bit, V_bit, N_bit, !Result, Result !== 0);
                break;
            case 'NOP':
                break;
            case 'OR':
                this.bitwiseOperation('OR');
                break;
            case 'ORI':
                this.bitwiseOperation('OR');
                break;
            case 'OUT':
                A_val = this.getArgValue(0);
                Rr = this.getArgValue(1);
                this.getDMEM().setValue(A_val + 0x20, Rr);   // I/O(A) <-- Rr
                break;
            case 'POP':
                if (this.getDMEM().getWord(0x5d) >= this.getDMEM().getRAMEnd()) return this.newError(`Bad stack pointer for POP on line ${this.line_in_file}.`);
                this.getDMEM().incSP();                                               // increment the SP by 1
                this.writeResultToRd(this.getDMEM().getValue(this.getDMEM().getWord(0x5d)));  // Rd <-- popped value
                this.getProgramData().offsetCycles(1);
                break;
            case 'PUSH':
                if (this.getDMEM().getWord(0x5d) <= 0x100) return this.newError(`Stack overflow. Bad stack pointer for PUSH on line ${this.line_in_file}.`);
                Rr = this.getArgValue(0);               // register held value
                this.getDMEM().setValue(this.getDMEM().getWord(0x5d), Rr);    // set the value in DMEM
                this.getDMEM().decSP();                           // decrement the SP by 1
                this.getProgramData().offsetCycles(1);
                break;
            case 'RCALL':
                this.call(2);
            ...
            case 'SBRS':
                ...
                break;
            case 'SEC':
                this.getDMEM().updateSREGBit(1, 0);
                break;
            case 'SEH':
                this.getDMEM().updateSREGBit(1, 5);
                break;
            case 'SEI':
                this.getDMEM().updateSREGBit(1, 7);
                break;
            case 'SEN':
                this.getDMEM().updateSREGBit(1, 2);
                break;
            case 'SER':
                this.writeResultToRd(0xff);   // Rd <-- 0xff
                break;
            case 'SES':
                this.getDMEM().updateSREGBit(1, 4);
                break;
            case 'SET':
                this.getDMEM().updateSREGBit(1, 6);
                break;
            case 'SEV':
                this.getDMEM().updateSREGBit(1, 3);
                break;
            case 'SEZ':
                this.getDMEM().updateSREGBit(1, 1);
                break;
            case 'ST':
                this.loadStore(1);
                break;
            case 'STD':
                this.loadStoreDisplacement(1);
                break;
            case 'STS':
                this.loadStoreDataSpace(1);
                break;
            case 'SUB':
                this.sub();
                break;
            ...
            case 'SWAP':
                ...
                this.writeResultToRd(Result);
                break;
            case 'TST':
                Result = this.getArgValue(0);   // Result = Rd
                N_bit = this.getBit(Result, 7);
                this.getDMEM().updateSREG(null, null, null, N_bit, 0, N_bit, !Result, null);
                break;
            case 'XCH':
                k = this.getDMEM().getWord(30);
                if ((k < 0x100) || (k > this.getDMEM().getRAMEnd())) this.newError(`Illegal value of Z pointer \'${k}\' on line ${this.line_in_file}.`);

                Rd = this.getArgValue(1);
                const reg_num = this.getRegNum(1);
                const k_val = this.getDMEM().getValue(k);
         
                this.getDMEM().setValue(k, Rd); // (Z) <-- Rd   and    Rd <-- (Z)
                this.getDMEM().setValue(reg_num, k_val);
                this.getProgramData().offsetCycles(1);
                break;
            default:
                break;
    ...
    run() {
        while (!this.getProgramData().isFinished()) {
            this.step();
        }
    }
    ...
    checkFinished() {
        ...
        if (this.getPC().getValue() === this.getProgramData().getBreakPoint()) this.getProgramData().setFinish(true);

        // Do nothing if it's finished running
        if (this.getProgramData().isFinished()) return true;        

        // If it should be finished, set it to finished then return
        if (this.getPC().getValue() >= this.getPMEM().getFlashEnd()) {
            console.log(`PC value too large. Number of steps taken: ${this.getProgramData().getStepCount()}`);
            this.getProgramData().setFinish(true);
            this.getPC().setValue(this.getPMEM().getFlashEnd());
            return true;
        }
    ...
    writeResultToRd(result, whole_word = 0) {
        this.getDMEM().setValue(this.getRegNum(0) + whole_word, result);
    }

    branch(s, clear_set, branch_arg = 0) {
        this.getProgramData().offsetBranchesSeen(1);
    ...
    skipBitReg(clear_set) {
        let Rd = this.getArgValue(0);
        let b = this.getArgValue(1);
    ...
    bitwiseOperation(inst) {
        // 0 = AND, 1 = CBR, 2 = OR, 3 = EOR, 4 = COM
        const Rd = this.getArgValue(0);
        let K_val;
        if (inst !== 'COM') K_val = this.getArgValue(1);
        if (inst === 'CBR') K_val = 0xff - K_val;
        
        let Result = Rd & K_val;    // assume it's an AND cause why not
        if      (inst === 'OR') Result = Rd | K_val;
        else if (inst === 'EOR') Result = Rd ^ K_val;
        else if (inst === 'COM') Result = 0xff - Rd;

        this.writeResultToRd(Result);   // Rd <-- Result

        const N_bit = this.getBit(Result, 7);
        const C_bit = (inst === 'COM') ? 1 : null;  // COM sets C to 1, the rest leave it unchanged
        this.getDMEM().updateSREG(null, null, null, N_bit, 0, N_bit, !Result, C_bit);
    }

    add(incl_carry = 0, Rr_is_Rd = 0) {
        const Rd = this.getArgValue(0);
        const Rr = this.getArgValue(1 - Rr_is_Rd);
        const C_bit = incl_carry & this.getDMEM().getSREGBit(0);
        const Result = this.mod256(Rd + Rr + C_bit);

        this.writeResultToRd(Result);   // Rd <-- Rd + Rr + C

        const H_bit = this.calcHBit(Result, Rd, Rr);
        const V_bit = ((Rd >= 128) & (Rr >= 128) & (Result < 128)) | ((Rd < 128) & (Rr < 128) & (Result >= 128));
        const N_bit = this.getBit(Result, 7);
        this.getDMEM().updateSREG(null, null, H_bit, N_bit ^ V_bit, V_bit, N_bit, !Result, (Rd + Rr + C_bit) > 255);
    }

    sub(incl_carry = 0, is_cp = 0) {
        const Rd = this.getArgValue(0);
        const K_val = this.getArgValue(1);
        const C_bit = (incl_carry) ? this.getDMEM().getSREGBit(0) : 0;
        const sub_val = K_val + C_bit;
        const Result = this.mod256(Rd - sub_val);

        if (!is_cp) this.writeResultToRd(Result);   // Rd <-- Rd - K - C (if C is used)

        const H_bit = this.calcHBit(Rd, K_val, Result);
        const V_bit = ((Result >= 128) & (K_val >= 128) & (Rd < 128)) | ((Result < 128) & (K_val < 128) & (Rd >= 128));
        const N_bit = this.getBit(Result, 7);
        const Z_bit = (incl_carry) ? (!Result) & this.getDMEM().getSREGBit(1): (!Result);
        const C_bit_new = (Rd - sub_val < 0);

        this.getDMEM().updateSREG(null, null, H_bit, N_bit ^ V_bit, V_bit, N_bit, Z_bit, C_bit_new);
    }

    immediateWithWord(is_sub = 0) {
        const Rd = this.getArgValue(0) + (this.getDMEM().getValue(this.getRegNum(0) + 1) << 8);
        const K_val = this.getArgValue(1);
        ...
        let V_bit = ((Result - K_val) < 0x8000) && (Result >= 0x8000);
        if (is_sub) V_bit = ((Result + K_val) >= 0x8000) && (Result < 0x8000);

        let C_bit = (is_sub) ? (Result + K_val) > 0xffff : (Result - K_val) < 0;   // if you went past 0xffff or 0 in the calc

        this.getDMEM().updateSREG(null, null, null, N_bit ^ V_bit, V_bit, N_bit, !Result, C_bit);
    }

    incDec(is_dec = 0) {
        const Rd = this.getArgValue(0);
        let Result = (is_dec) ? Rd - 1 : Rd + 1;
        Result = this.mod256(Result);

        this.writeResultToRd(Result);   // Rd <-- Rd +- 1

        const V_bit = ( Result === (128 - is_dec) );
        const N_bit = this.getBit(Result, 7);
        this.getDMEM().updateSREG(null, null, null, N_bit ^ V_bit, V_bit, N_bit, !Result, null);
    }

    mul(num_signed = 0, is_frac = 0) {
        let Rd = this.getArgValue(0);
        if (num_signed) Rd = (Rd < 128) ? Rd : Rd - 256;    // convert to signed
        let Rr = this.getArgValue(1);
        if (num_signed > 1) Rr = (Rr < 128) ? Rr : Rr - 256;    // convert to signed
        let Result = Rd * Rr;
        if (num_signed) Result = (Result < 0) ? 65536 + Result : Result;        // convert to unsigned equivalent
        if (is_frac) Result *= 2;
        
        this.getDMEM().setValue(0, this.mod256(Result));
        this.getDMEM().setValue(1, this.mod256(Result >> 8));

        const C_shift = 15 + is_frac;
        this.getDMEM().updateSREG(null, null, null, null, null, null, !Result, (Result >> C_shift) & 1);

        if (num_signed === 1) this.getProgramData().offsetCycles(1);  // for MULSU and FMULSU
    }

    shiftRight(shift_type) {        
        // LSR = 0, ASR = 1, ROR = 2
        const Rd = this.getArgValue(0);

        const bit7 = [0, this.getBit(Rd, 7), this.getDMEM().getSREGBit(0)][shift_type];

        const Result = this.mod256(Rd >> 1) + (bit7 << 7);

        this.writeResultToRd(Result);

        const N_bit = (!shift_type) ? 0 : this.getBit(Result, 7);
        const C_bit = Rd & 1;
        const V_bit = N_bit ^ C_bit;
        this.getDMEM().updateSREG(null, null, null, N_bit ^ V_bit, V_bit, N_bit, !Result, C_bit);
    }
    ...
    call(call_type = 0) {
        ...
        if (call_type) return;
        this.getProgramData().offsetCycles(1);   // CALL takes 1 more cycle than the others
    }

    ret() {
        const sp = this.getDMEM().getWord(0x5d);

        // Finish program
        if (sp === this.getDMEM().getRAMEnd()) {
            this.getProgramData().setFinish(true);
            this.getProgramData().offsetStepCount(1);
            this.getProgramData().offsetCycles(5);
            console.log(`Number of steps taken: ${this.getProgramData().getStepCount()}`);
            return true;
        }

        if ((sp < 0x100) || (sp > (this.getDMEM().getRAMEnd() - 2))) {
            return this.newError(`Bad stack pointer for RET on line ${this.line_in_file}.`);
        }

        // Get the return line from the stack
        this.getDMEM().incSP();
        let ret_line = (this.getDMEM().getValue(this.getDMEM().getWord(0x5d)) << 8); // get the ret high value

        this.getDMEM().incSP();
        ret_line += this.getDMEM().getValue(this.getDMEM().getWord(0x5d)); // add the ret low value to the ret high value

        this.getPC().setValue(ret_line - 1);
        this.getProgramData().offsetCycles(4);
        return false;
    }

    loadStoreDataSpace(is_sts = 0) {
        // LDS = 0, STS = 1
        const k = this.getArgValue(1 - is_sts);
        const Rd = this.getArgValue(1);
        
        if (is_sts) this.getDMEM().setValue(k, Rd);                     // (k) <-- Rd
        else        this.writeResultToRd(this.getDMEM().getValue(k));   // Rd <-- (k)
        
        this.getPC().offsetValue(1); // increment once now cause total needs to be + 2
        this.getProgramData().offsetCycles(1);
    }

    loadStore(is_st = 0) {
        let w = this.getArgValue(1 - is_st);
        const plusminus = -1 * w.includes('-') + w.includes('+'); // get if it's got a + or -
        
        w = w.replace('-', '').replace('+', '');                  // make it X, Y, or Z only
        const low_byte = (['X', 'Y', 'Z'].indexOf(w) * 2) + 26;   // convert it to a number

        // Decrement X/Y/Z
        if (plusminus === -1) this.getDMEM().offsetWord(low_byte, -1);
        
        const k = this.getDMEM().getWord(low_byte);

        const illegal_k = (k < 0x100 || k > this.getDMEM().getRAMEnd());
        if (illegal_k) this.newError(`Illegal value \'${k}\' for ${w} on line ${this.line_in_file}.`);

        if (is_st) this.getDMEM().setValue(k, this.getArgValue(1));  // (k) <-- Rr
        
        if (plusminus === 1) this.getDMEM().offsetWord(low_byte, 1);

        const offset = [2, 0, 1][plusminus + 1];    // +2 for -1, +0 for 0, +1 for 1 
        this.getProgramData().offsetCycles(offset);
        
        if (!is_st) this.writeResultToRd(this.getDMEM().getValue(k));  // Rd <-- (k)
    }

    loadStoreDisplacement(is_st = 0) {
        //STD
        const w = this.getRegNum(1 - is_st)[0]; // TODO check this
        const q = parseInt(this.getRegNum(1 - is_st).slice(2)); // TODO check this

        let k;
        if (w === 'Y') k = this.getDMEM().getWord(28) + q;
        else if (w === 'Z') k = this.getDMEM().getWord(30) + q;
        
        if (k < 0x100 || k > this.getDMEM().getRAMEnd()) this.newError(`Illegal value \'${k}\' for ${w}+${q} on line ${this.line_in_file}.`);
        
        this.getProgramData().offsetCycles(1);
        
        if (!is_st) return this.writeResultToRd(this.getDMEM().getValue(k));  // Rd <-- (k)

        const Rd = this.getArgValue(1);
        this.getDMEM().setValue(k, Rd);   // (k) <-- Rd
    }
    ...
    mov(is_word = 0) {
        ...
        Rr = this.getDMEM().getValue(this.getRegNum(1) + 1);
        this.writeResultToRd(Rr, 1);    // Rd <-- Rr
    }

    pushPCToStack() {
        this.getDMEM().setValue(this.getDMEM().getWord(0x5d), this.getPC().getPCL().getValue());    // push pcl in STACK
        this.getDMEM().decSP();
        this.getDMEM().setValue(this.getDMEM().getWord(0x5d), this.getPC().getPCH().getValue());    // push pch in STACK
        this.getDMEM().decSP();
    }
    ...
    printf() {
        ...
        2) Puts them into X low byte then high byte (must push high byte first to call printf)
        3) Assumes that it is pointing at the string you want to print
        4) Prints characters with X+ until it hits a null character
        5) Returns
        ...
        */
        ...
        const can_pop_twice = (sp <= this.getDMEM().getRAMEnd() - 2);
        if (!can_pop_twice) return this.newError(`Bad stack pointer for CALL on line ${this.line_in_file}.`);
        
        let address = this.getDMEM().getValue(sp + 1) + ( this.getDMEM().getValue(sp + 2) << 8);

        // Printf just does this
        this.getDMEM().setMultipleValues([26, 27, 30, 31], [0xff, 0x08, 0x02, 0x01]);
        
        // Do the printing
        let W_val = this.printToConsole(address);

        this.getDMEM().setValue(24, this.mod256(W_val));
    ...
    printToConsole(start_address) {
        let address = start_address;
        let W_val = 0;  // stores the length of the string
        let K_val = this.getDMEM().getValue(address);
        let char = String.fromCharCode(K_val);

        while (K_val !== 0) {   // break at a null character
            address += 1;
            W_val += 1;

            if (this.getProgramData().getPrint()) document.getElementById('console').innerHTML += char;   // add it to the console

            if (address > this.getDMEM().getRAMEnd()) break;    // don't go beyond the end of memory

            K_val = this.getDMEM().getValue(address);
    ...
    bitInIO(clear_set, skip) {
        // 0 = clear, 1 = set
        const A_val = this.getArgValue(0);
        const b = this.getArgValue(1);
        const dmem_val = this.getDMEM().getValue(A_val + 0x20);
        this.getProgramData().offsetCycles(1);

        if (skip) {
            if (!(clear_set) && ((dmem_val >> b) & 1)) return;  // return if it's set but should be cleared
            if ((clear_set) && !((dmem_val >> b) & 1)) return;  // return if it's cleared but should be set
            return this.skip();
        }

        const bit_val = (1 << b);
        const Result = (clear_set) ? dmem_val | bit_val : dmem_val & (0xff - bit_val);
        this.getDMEM().setValue(A_val + 0x20, Result);
        return;

    }

    incrementStepCount() {
        this.getProgramData().offsetStepCount(1);   // count the number of steps to prevent infinite loops
        this.getProgramData().offsetCycles(1); // count the number of clock cycles used

        // If the number of steps is too large, terminate running the code
        if (this.getProgramData().getStepCount() > (1 << 20)) {
            return this.newError('Number of steps in code too large. Execution terminated.');
        } 
    }
    ...
    getNotBit(value, bit) {
        // returns the inverted value of a bit in a number
        return 1 - this.getBit(value, bit);
    }

    getLine(offset = 0) {
        const line_num = this.getPC().getValue() + offset;
        if (line_num >= this.getPMEM().getFlashEnd()) {
            return this.newError(`Cannot access PMEM line ${line_num} on line ${this.line_in_file}.`);
        }
        return this.getPMEM().getValue(line_num);
    }
    ...
    getArgValue(arg_num) {
        ...
        if (reqs.includes('REG')) return this.getDMEM().getValue(arg.getValue());
        return arg.getValue();
    }

    calcHBit(inverted_value, other_val1, other_val2) {
        const inv_or_1 = this.getNotBit(inverted_value, 3) & this.getBit(other_val1, 3);
        const inv_or_2 = this.getNotBit(inverted_value, 3) & this.getBit(other_val2, 3);
        const other_or = this.getBit(other_val1, 3)        & this.getBit(other_val2, 3);

        return inv_or_1 | inv_or_2 | other_or;
    }
...
class DMEM {
    ...
    setMultipleValues(loc_arr, val_arr) {
        for (let i = 0; i < loc_arr.length; i++) {
            this.setValue(loc_arr[i], val_arr[i]);
        }
    }

    getWord(low_reg) {
        return (this.getValue(low_reg + 1) << 8) + this.getValue(low_reg);
    }

    offsetWord(low_reg, offset) {
        const val = this.getWord(low_reg) + offset;
        this.setValue(low_reg, this.mod256(val));
        this.setValue(low_reg + 1, this.mod256(val >> 8));
    }

    getSREGBit(bit) {
        return (this.getValue(0x5f) >> bit) & 1;
    }

    updateSREGBit(value, bit) {
        this.getValue()[0x5f].updateBit(value, bit);
    }
    ...
    setSP(new_value) {
        const hi8 = this.mod256(new_value >> 8);
        const lo8 = this.mod256(new_value);
        this.setValue(0x5e, hi8);
        this.setValue(0x5d, lo8);
    }

    incSP() {
        this.setSP(this.getWord(0x5d) + 1);
    }
...
class PMEM {
    ...
    getValue(loc, line) {
        if (loc >= this.getLength()) return this.newError(`Illegal PMEM location: ${loc}. Execution terminated.`);
        return this.memory[loc];
    }

    setValue(loc, val, line) {
        if (loc >= this.getLength()) return this.newError(`Illegal PMEM location: ${loc}. Execution terminated.`);
        this.memory[loc] = val;
    }
...
class ProgramData {
    constructor(break_point = null, print = true) {
        this.step_count = 0;
        this.cycles = 0;
    ...
    offsetStepCount(offset) {
        this.step_count += offset;
    }
    ...
    getBranchesTaken() {
        return this.branches_taken;
    }

    offsetBranchesTaken(offset) {
        this.branches_taken += offset;
    }

    getBranchesSeen() {
        return this.branches_seen;
    }

    offsetBranchesSeen(offset) {
        this.branches_seen += offset;
    }
...
class ProgramDefinitions {
    ...
    newEqu(label, value, line_in_file) {
        ...
        this.getEqus()[label] = value;
    }

    newDef(label, value, line_in_file) {
        // If its already a label or .equ
        if ( this.labelExists(label) || this.equExists(label) ) {
        ...
        this.getDefs()[label] = value;
    }

    labelExists(label) {
        return this.getLabels()[label] !== undefined;
    }

    equExists(label) {
        return this.getEqus()[label] !== undefined;
    }

    defExists(label) {
        return this.getDefs()[label] !== undefined;
    }

    instExists(label) {
        return this.getInstSet().isInstruction(label);
    }
...
class BreakPoint {
    constructor() {
        this.value = null;    // for stopping the interpreter run() method at
        this.line_in_file = parseInt(document.getElementById('break_point').value);
    }
    ...
    defineBreakPoint(val, line_in_file) {
        // If the break point should be here set the break point to be the line about to be put in
        if (Number.isNaN(this.getLineInFile())) return
        if (line_in_file < this.getLineInFile()) return;
...
class LineManager {
    constructor() {
        this.all_lines = [];   // Tokens in each line
    ...
    getAllLines(line_num = null) {
        if (line_num === null) return this.all_lines;
        return this.all_lines[line_num];
    }

    getNumLines() {
        return this.all_lines.length;
    }

    getLine(loc = null) {
        if (loc === null) return this.line;
        return this.line[loc];
    }
    ...
    setLine(line = null, line_num = null) {
        if (line !== null) this.line = line;
        else if (line_num !== null) this.line = this.getAllLines(line_num);
        else this.line = this.getAllLines(this.getLineNumber());
    }
    ...
    offsetLineNumber(val) {
        this.line_num += val;
    }

    getTextSecStart() {
        return this.text_section_start;
    }

    setTextSecStart(val) {
        this.text_section_start = val;
    }

    getLineInFile() {
        return this.getLine(0).getLine();
    }
...
class TokenManager {
    constructor() {
        this.current_tok = null;
        this.tok_num = 0;
        this.line_manager = new LineManager();
    }
    ...
    setToken(loc = null) {
        const place = (loc !== null) ? loc : this.getTokNum();
        const tok = this.getLineManager().getLine(place);
        this.current_tok = tok;
    }

    setValue(val) {
        this.getToken().setValue(val);
    }

    setType(typ) {
        this.getToken().setType(typ);
    }
}
...
class Operands {
    constructor() {
        ...
        const reg_16_23 = new Argument(['REG','INT'], 16, 23);
        ...
        const int_n2048_2047 = new Argument('INT', -2048, 2047);
        const word_plus_q_0_63 = new Argument('WORDPLUSQ', 0, 63);
        const word_wxyz = new Argument(['REG','INT'], null, null, [24, 26, 28, 30]);
        const word_var = new Argument(['WORD', 'MINUSWORD', 'WORDPLUS']);
        const ram_val = new Argument('INT', 0x100, 0x8ff); // ramend = 0x8ff
        ...
        this.inst_operands = {
            ...
            'CALL': [new Argument(['INT', 'REF'], 0, 0x3fff, this.functions.getFunctions())], // flashend = 0x3fff
            ...
            'SBIC': [int_0_31, int_0_7],
            'SBIS': [int_0_31, int_0_7],
            ...
            'SEV': null,
            'SEZ': null,
            'ST': [word_var, reg_0_31],
            'STD': [word_plus_q_0_63, reg_0_31],
            'STS': [ram_val, reg_0_31],
...
class StatusHandler {
    constructor() {
        this.success_colour = "#b0f0b0";
        this.error_colour = "#f0b0b0";
    }
    ...
    success(text) {
        ...
        this.flash(this.success_colour, 700);
    }
    ...
    newError(text) {
        ...
        this.setinnerHTML('output', null);
        this.setinnerHTML('status', null);
        this.flash(this.error_colour, 700);
...
class EventListeners {
    ...
    codeBoxSupports() {
        ...
        code_box.addEventListener('keyup', event => {
            ...
            const lines_array = Array.from({ length: line_count }, (_, i) => String(i + 1));

            const lines_box = document.getElementById('lines_box');
            lines_box.innerHTML = lines_array.join('\n');
        });
...
class App {
    ...
    assemble(print = true) {
        ...
        // Parsing
        this.parser.newData(this.lexer.getTokenManager(), txt);

        // Interpreter Initialisation
        this.interpreter.newData(this.parser.getPMEM(), this.parser.getDMEM(), this.parser.getBreakPoint().getValue());

        // Success!
        this.success('Success! Your code can be run.', print);

        // Populating Display with Data
        this.pmem_top = this.interpreter.getPC().getValue() - ( this.interpreter.getPC().getValue() % 8 );
        this.populateAll();
    }
    ...
    progress(prog_type = 0) {
        ...
        const current_pc = this.interpreter.getPC().getValue();
        this.pmem_top = current_pc - (current_pc % 8); // move pmem display to the line

        if (this.interpreter.getProgramData().isFinished()) {
            if (this.interpreter.getProgramData().getBreakPoint() !== current_pc) this.success('The code has run and exited successfully!');
            else this.success(`The code has run and exited successfully at the breakpoint on line ${this.parser.getBreakPoint().getLineInFile()}!`);
        }
    ...
    async runSlowly() {
        ...
        let sleep_time;
        let t0 = Date.now();
        while (!this.interpreter.getProgramData().isFinished() && !this.pause) {
    ...
    stepBack() {
        ...
        const steps = this.interpreter.getProgramData().getStepCount() - steps_back;       // the total number of steps to get to the point you want to go for
        ...
        this.interpreter.getProgramData().setPrint(false);   // stop from printing
        ...
        this.pmem_top = this.interpreter.getPC().getValue() - (this.interpreter.getPC().getValue() % 8); // move pmem display to the line
        
        this.interpreter.getProgramData().setPrint(true);    // allow printing again
    ...
    populateRegisters() {
        ...
        const registers = this.interpreter.getDMEM().getValue().slice(0, 32);
    ...
    populateSREG() {
        ...
        let change = this.interpreter.getDMEM().getValue()[0x5f].getBitsChanged();    // the bits that have been changed

        const sreg_flags = ['C', 'Z', 'N', 'V', 'S', 'H', 'T', 'I'];
        for (let i = 0; i < 8; i++) {
            const flag = sreg_flags[i];

            const flag_value = this.interpreter.getDMEM().getValue()[0x5f].getBit(i) ? '1' : '0';
            
            this.setinnerHTML(`sreg-${flag}`, flag_value); // set the inner HTML

            // If it's changed, make the display different
            if (this.interpreter.getDMEM().getValue()[0x5f].getChange() && ((change >> i) & 1)) {
    ...
    populatePointers() {
        ...
        const pc = this.convertValueToBase(this.interpreter.getPC().getValue(), 4);
        const sp = this.convertValueToBase(this.interpreter.getDMEM().getWord(0x5d), 4);
        const x = this.convertValueToBase(this.interpreter.getDMEM().getWord(26), 4);
        const y = this.convertValueToBase(this.interpreter.getDMEM().getWord(28), 4);
        const z = this.convertValueToBase(this.interpreter.getDMEM().getWord(30), 4);
    ...
    populatePMEM(start_cell) {
        ...
        const pc = this.interpreter.getPC().getValue();
        ...
        for (let line = 0; line < num_lines; line++) {
            ...
            let line_value = this.interpreter.getPMEM().getValue(start_cell + line);  // get the line to print

            // If the opcode display = true 
            if (this.display_opcode) {
                if (line_value !== null) line_value = line_value.getOpcode().slice(0, 16);              // replace null lines with (two line inst.)
                else                     line_value = this.interpreter.getPMEM().getValue(start_cell + line - 1).getOpcode().slice(16);  
            }

            // If the instruction is to be display, not the opcode
            else {
                if (line_value !== null) line_value = line_value.toString(this.base);                   // replace null lines with (two line inst.)
    ...
    populateDMEM(start_cell) {
        ...
        const sp = this.interpreter.getDMEM().getWord(0x5d);
        const x = this.interpreter.getDMEM().getWord(26);
        const y = this.interpreter.getDMEM().getWord(28);
        const z = this.interpreter.getDMEM().getWord(30);
    ...
    populateStats() {
        ...
        this.setinnerHTML('stats-instructions-executed', this.interpreter.getProgramData().getStepCount());
        this.setinnerHTML('stats-clock-cycles', this.interpreter.getProgramData().getCycles());
        this.setinnerHTML('stats-branches-seen', this.interpreter.getProgramData().getBranchesSeen());
        this.setinnerHTML('stats-branches-taken', this.interpreter.getProgramData().getBranchesTaken());
    }
    ...
    displayPMEMDown() {
        ...
        if (this.pmem_top > (this.parser.getPMEM().getFlashEnd() - 8)) return;
    ...
    displayDMEMDown() {
        ...
        if (this.dmem_top > (this.parser.getDMEM().getRAMEnd() - 0x40)) return;
    ...
    displayDMEMBottom() {
        this.hideOpenPopup(); // hide any open popup
        this.dmem_top = (this.parser.getDMEM().getRAMEnd() - 0x3f);
        this.populateDMEM(this.dmem_top);
    }
    ...
    fillPopup() {
        ...
        let inst = null;
        if (['PC', 'SP', 'X', 'Y', 'Z'].includes(this.current_popup)) popup.innerHTML = '';
        else {
    ...
    getPopups() {
        const popups = {
            ...
            'SBI': `Syntax:   SBI A, b
                    ...
                    Operation:
                    I/O(A, b) = 1`,
            'SBIC': `Syntax:   SBIC A, b
                    Family:   Branch Instructions
                    Function: Tests a single bit in an I/O register and skips the next instruction if the bit is cleared
                    ...
                    Operation Options:
                    PC = PC + 1 { if I/O(A, b) = 1 }
                    PC = PC + 2 { if I/O(A, b) = 0 and the next instruction is 16 bits }
                    PC = PC + 3 { if I/O(A, b) = 0 and the next instruction is 32 bits }`,
            'SBIS': `Syntax:   SBIS A, b
                    Family:   Branch Instructions
                    Function: Tests a single bit in an I/O register and skips the next instruction if the bit is set
                    ...
                    Operation Options:
                    PC = PC + 1 { if I/O(A, b) = 0 }
                    PC = PC + 2 { if I/O(A, b) = 1 and the next instruction is 16 bits }
                    PC = PC + 3 { if I/O(A, b) = 1 and the next instruction is 32 bits }`,
